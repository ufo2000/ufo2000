...
function process_log(filename, history)
    ...
    local function calculate_elo(name1, name2, winner)
        if winner ~= name1 then
            tempname = name1
            name1 = name2
            name2 = tempname
        end
        if tournament_table[name1] and tournament_table[name2] then
            expected1 = 1 / (1 + 10 ^ ((tournament_table[name2].elo_score - tournament_table[name1].elo_score) / 400))
            expected2 = 1 / (1 + 10 ^ ((tournament_table[name1].elo_score - tournament_table[name2].elo_score) / 400))
            if tournament_table[name1].elo_score <= 2400 then k = 32 else k = 16 end
            tournament_table[name1].elo_score = tournament_table[name1].elo_score + (k * (1 - expected1))
            if tournament_table[name2].elo_score <= 2400 then k = 32 else k = 16 end
            tournament_table[name2].elo_score = tournament_table[name2].elo_score + (k * (0 - expected2))
        end
    end
    ...
    local function endgame(p, l)
        ...
        if game_info.exited[game_info.p1] and game_info.exited[game_info.p2] then
            ...
            if game_info.winner then
                ...
                calculate_elo(game_info.p1, game_info.p2, game_info.winner)
            end
...
out:write("<tr><td>rank<td>name<td>games played<td>games won<td>server chat time<td>battle time<td>score<td>ELO score\n")
local tmp = {}
...
-- use the line below to sort using score
--table.sort(tmp, function(a, b) return a[2].score > b[2].score end)

-- use the line below to sort using elo score
table.sort(tmp, function(a, b) return a[2].elo_score > b[2].elo_score end)

tournament_table = tmp

for index, data in tournament_table do
    out:write(string.format(
        "<tr><td>%d<td>%s<td>%d<td>%d<td>%s<td>%s<td>%d<td>%d\n",
        index,
        data[1],
