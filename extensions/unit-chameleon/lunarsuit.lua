...
for row = 7, 0, -1 do
    for col = 0, 20 do
        table.insert(frames, png_image_fragment("$(extension)/lunarsuit.png",
...
for row = 0, 7, 1 do
    table.insert(falling_frames, png_image_fragment("$(extension)/lunarsuit.png",
         298, 1 + row * 41, 32, 40))
end
...
local function sprite_renderer(img, dir, phase, info)
    ...
    local delta_y = -4
    if info.is_crouching then delta_y = -1 end
...
AddUnit {
    ...
    pInv = png_image_fragment("$(extension)/lunarsuit_inv.png", 0, 0, 74, 106)
}
