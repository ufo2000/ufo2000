...
bool Scenario::platoon_common (long points, Platoon *platoon, PanPos pos, char buf[10000], int len)
{
			    ...
			    for (int k = 0; k < 6; k++) {
			        if (buf[i] == explosives[j][k]) {
			            if (type != SC_SABOTAGE || explosives[j][k] != HIGH_EXPLOSIVE || pos != POS_LEFT) {
                            g_console->printf(COLOR_RED04, _("%s isn't allowed."), Item::obdata_name(explosives[j][k]).c_str());
			            	return false;
						}
					}
				}
	...
	if (!rules[4]) {
		...
		if (len2 > 0) {
            g_console->printf(COLOR_RED04, _("Items on the ground aren't allowed (you have: %d, including unselected soldiers)."), len2);
			return false;
		}
	}
...
bool Scenario::platoon_capture (Platoon *platoon, char *first_soldier, PanPos pos, char buf[10000], int len)
{
	if (pos == POS_LEFT) {
		...
		if (!stun_rod && (!stun_gun || !stun_bomb)) {
            g_console->printf(COLOR_RED04, _("You must have at least one stun rod or one loaded stun launcher.") );
			return false;
		}
	} else {
