...
public:
    ...
    static int CreateGame(std::string playername1,std::string playername2);
    ...
    Server_Game_UFO* ActivatePlayer(int game_id,ServerClientUfo* player);
    //! The user is getting offline, if his opponent is offline, the game
    //! object can be deleted till a next user activation.
    Server_Game_UFO* DeactivatePlayer(int game_id,ServerClientUfo* player);
    //! Packet from user
    int PacketToServer(ServerClientUfo* sender,const std::string &packet);
private:
	Server_Game_UFO() : db_conn(DB_FILENAME) {
        players[0] = NULL;
        players[1] = NULL;
    }
    //! Pointers to clients, NULL if a client is not active
    ServerClientUfo *players[2];
    static std::map<std::string, Server_Game_UFO *> active_games;
    int PacketToClient(ServerClientUfo* recipient,const std::string &packet);
